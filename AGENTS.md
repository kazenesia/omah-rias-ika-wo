...
- Located in the `docs/directives/` folder as Markdown files.
...
- You are the bridge. Instead of executing complex tasks (like web scraping) directly, you parse the directive and trigger the corresponding script (e.g., `scripts/web_scraper.py`).


**Tier 3: The Muscle (Execution)**

- Located in the `scripts/` folder as Python scripts.
...
Always check the `scripts/` folder for existing scripts before writing new code. Avoid redundant tool creation.
...
Directives are living documents. Whenever you discover a better workflow, API limitation, or common bug, update the corresponding file in `docs/directives/`. However, never overwrite or delete a directive entirely without explicit permission. Your instructions must be preserved and improved over time.
...
- `scripts/` - The deterministic Python scripts.

- `docs/directives/` - Your Markdown instruction manuals.
